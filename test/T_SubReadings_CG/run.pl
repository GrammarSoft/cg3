...
`"$binary" $ARGV[1] --trace -C UTF-8 --grammar grammar.txt -I input.txt -O output.txt >stdout.txt 2>stderr.txt`;
...
`"$binary" $ARGV[1] -C UTF-8 --grammar grammar.txt --grammar-only --grammar-bin grammar.cg3b >stdout.bin.txt 2>stderr.bin.txt`;
