...
`"$binary" $ARGV[1] --trace -C ISO-8859-1 --grammar grammar.txt -I input.txt -O output.txt >stdout.txt 2>stderr.txt`;
...
`"$binary" $ARGV[1] --trace -C ISO-8859-1 --grammar grammar.txt --grammar-only --grammar-bin grammar.cg3b >stdout.bin.txt 2>stderr.bin.txt`;
