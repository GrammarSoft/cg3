...
function profile_revision($rev) {
	...
	for ($i=0 ; $i<3 ; $i++) {
		...
		echo "Applying...\n";
		$start = microtime(true);
		$time = shell_exec('head -n 2000 /vislcg3/trunk/comparison/arboretum_stripped.txt | /usr/bin/time ./vislcg3 -C ISO-8859-1 -g dancg.cg3b 2>&1 | grep user | grep system');
	...
	$ticks = shell_exec('head -n 2000 /vislcg3/trunk/comparison/arboretum_stripped.txt | valgrind ./vislcg3 -C ISO-8859-1 -g dancg.cg3b 2>&1 | grep "total heap usage"');
	...
	$ticks = shell_exec('head -n 2000 /vislcg3/trunk/comparison/arboretum_stripped.txt | valgrind --tool=callgrind --compress-strings=no --compress-pos=no --collect-jumps=yes --collect-systime=yes ./vislcg3 -C ISO-8859-1 -g dancg.cg3b 2>&1 | grep Collected');
...
$revs = array(5930, 5926, 5918, 5839, 5810, 5773, 5729, 5431, 5129, 5042, 4879, 4779, 4545, 4513, 4493, 4474, 4410, 4292, 4031, 3991, 3896, 3852, 3800, 3689, 3682, 3617);
foreach ($revs as $rev) {
